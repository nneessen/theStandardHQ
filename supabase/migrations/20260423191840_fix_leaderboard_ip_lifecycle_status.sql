-- supabase/migrations/20260423191840_fix_leaderboard_ip_lifecycle_status.sql
-- Fix: IP leaderboards counted lapsed/cancelled/expired policies as "Issued Paid"
--
-- Problem:
--   After the 2026-02-04 decoupling migration, status='approved' no longer means
--   "in force" -- lapsed/cancelled/expired policies all carry status='approved' +
--   a non-active lifecycle_status. The leaderboard IP CTEs still filtered on
--   p.status='approved', so an agent's IP included premium from policies that
--   had already lapsed or been cancelled.
--
-- Fix:
--   Swap `p.status = 'approved'` -> `p.lifecycle_status = 'active'` in the IP CTE
--   of each leaderboard RPC. Only in-force policies now count toward IP.
--   'denied' and 'withdrawn' remain excluded (both have lifecycle_status = NULL).
--   'pending' remains excluded for the same reason.
--
-- Bonus: the new filter matches the partial index
--   `idx_policies_imo_lifecycle_effective WHERE lifecycle_status='active'`,
--   which should reduce IP query cost.
--
-- Affected functions (IP CTE only; all other CTEs untouched):
--   - get_leaderboard_data            (Agents)
--   - get_agency_leaderboard_data     (Agencies)
--   - get_team_leaderboard_data       (Teams)
--
-- Not touched:
--   - get_submit_leaderboard          (ranks by AP, not IP)
--   - AP CTEs across all three above  (AP semantics confirmed correct as-is)
...
BEGIN
  ...
  -- IP: in-force policies (lifecycle_status='active') with effective_date in range
  ip_data AS (
    SELECT
      p.user_id,
...
COMMENT ON FUNCTION get_leaderboard_data IS 'Individual agent leaderboard with IMO tenant isolation. IP = in-force policies (lifecycle_status=active) by effective_date. AP = ALL submitted policies by submit_date. Scoped to caller''s IMO via get_my_imo_id().';

INSERT INTO supabase_migrations.function_versions (function_name, current_version)
VALUES ('get_leaderboard_data', '20260423191840')
...
COMMENT ON FUNCTION get_agency_leaderboard_data IS 'Agency leaderboard with IMO tenant isolation and MLM hierarchy aggregation. IP = in-force policies (lifecycle_status=active). Includes agents via hierarchy_path OR agency_id descendants. Scoped to caller''s IMO via get_my_imo_id().';

INSERT INTO supabase_migrations.function_versions (function_name, current_version)
VALUES ('get_agency_leaderboard_data', '20260423191840')
ON CONFLICT (function_name) DO UPDATE SET
  current_version = EXCLUDED.current_version,
  updated_at = NOW();
...
COMMENT ON FUNCTION get_team_leaderboard_data IS 'Team leaderboard with IMO tenant isolation and full MLM hierarchy aggregation. IP = in-force policies (lifecycle_status=active). Scoped to caller''s IMO via get_my_imo_id().';

INSERT INTO supabase_migrations.function_versions (function_name, current_version)
VALUES ('get_team_leaderboard_data', '20260423191840')
ON CONFLICT (function_name) DO UPDATE SET
  current_version = EXCLUDED.current_version,
  updated_at = NOW();
