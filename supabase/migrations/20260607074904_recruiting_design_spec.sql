-- Migration: AI-composed recruiting landing pages
-- Adds design_spec (jsonb) + design_prompt (text) to recruiting_page_settings and
-- surfaces design_spec through the public theme RPC.
--
-- design_spec holds the validated structured "block composer" design produced by
-- the generate-recruiting-design edge function. NULL = render via the legacy theme
-- columns (legacyThemeToSpec on the client). The spec is re-validated client-side
-- on every public render, so it is never trusted at rest.

-- 1. Columns (additive, nullable; no CHECK constraints — enums enforced in TS).
...
COMMENT ON COLUMN public.recruiting_page_settings.design_spec IS
  'AI-generated validated structured design spec (discriminated-union blocks + theme). NULL = render via legacy theme columns. Re-validated client-side before render.';
COMMENT ON COLUMN public.recruiting_page_settings.design_prompt IS
  'The agent''s last AI design prompt, kept for re-editing context. Cosmetic only.';

-- 2. Re-issue get_public_recruiting_theme — body copied verbatim from
--    20260527114910_revocation_public_surface_gate.sql (ALL sunset gates intact),
--    adding ONE key to the returned JSON: design_spec. New filename timestamp
--    (20260607...) > 20260527114910 so the runner classifies this as an upgrade.
...
BEGIN
  ...
  v_result := json_build_object(
    ...
    'design_spec', v_settings.design_spec   -- NEW: user-only; NULL signals legacy fallback
  );
