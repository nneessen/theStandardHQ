...
DROP FUNCTION IF EXISTS public.persist_underwriting_run_v1(JSONB, JSONB, JSONB);
...
COMMENT ON FUNCTION public.persist_underwriting_run_v1(UUID, JSONB, JSONB, JSONB) IS
'Service-role-only authoritative persistence path for UW Wizard runs. The edge function verifies the JWT and passes the actor UUID explicitly so clients cannot call this RPC directly with forged result payloads.';

REVOKE ALL ON FUNCTION public.persist_underwriting_run_v1(UUID, JSONB, JSONB, JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.persist_underwriting_run_v1(UUID, JSONB, JSONB, JSONB) TO service_role;
