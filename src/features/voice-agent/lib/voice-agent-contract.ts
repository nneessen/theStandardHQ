...
// ─── Phone Number Version Update ────────────────────────────────
// Retell requires agent IDs in the PATCH — without them, setting version
// to null unbinds the phone number entirely. Version null resolves to the
// current latest at call time (Retell has no persistent "always latest" mode).
const PHONE_NUMBER_UPDATE_ALLOWED_KEYS = new Set([
  "phoneNumber",
  "inbound_agent_id",
  "outbound_agent_id",
...
export function parseUpdatePhoneNumberParams(params: Record<string, unknown>) {
  const invalidKeys = Object.keys(params).filter(
    (key) => !PHONE_NUMBER_UPDATE_ALLOWED_KEYS.has(key),
  );
  if (invalidKeys.length > 0) {
    throw new Error(
      `Phone number update contains unsupported fields: ${invalidKeys.join(", ")}.`,
    );
  }

  const phoneNumber = readRequiredTrimmedString(
    params.phoneNumber,
    "Phone number",
  );
  assertPhoneNumber(phoneNumber, "Phone number");

  const inboundAgentId = readOptionalTrimmedString(
    params.inbound_agent_id,
    "Inbound agent ID",
  );
  const outboundAgentId = readOptionalTrimmedString(
    params.outbound_agent_id,
    "Outbound agent ID",
  );

  // null = resolve to latest at call time; number = pin to specific version
  const inboundVersion = params.inbound_agent_version;
  if (
    inboundVersion !== undefined &&
    inboundVersion !== null &&
    typeof inboundVersion !== "number"
  ) {
    throw new Error("inbound_agent_version must be a number or null.");
  }

  const outboundVersion = params.outbound_agent_version;
  if (
    outboundVersion !== undefined &&
    outboundVersion !== null &&
    typeof outboundVersion !== "number"
  ) {
    throw new Error("outbound_agent_version must be a number or null.");
  }

  return {
    phoneNumber,
    ...(inboundAgentId ? { inbound_agent_id: inboundAgentId } : {}),
    ...(outboundAgentId ? { outbound_agent_id: outboundAgentId } : {}),
    ...(inboundVersion !== undefined
