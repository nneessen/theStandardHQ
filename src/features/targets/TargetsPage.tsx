...
import { SimpleTargetsView } from "./components/SimpleTargetsView";
import { PercentInput } from "./components/PercentInput";
import { AvgPremiumField } from "./components/AvgPremiumField";
import {
  Board,
  Cap,
  Pill,
...
export function TargetsPage() {
  ...
  // Per-agent avg-premium override (this user's own value, set on this page).
  // When set (> 0) it drives the divisor in both plans and supersedes the
  // Mean/Median toggle; otherwise the computed cohort average is used.
  const avgPremiumOverride =
    targets?.avgPremiumOverride && targets.avgPremiumOverride > 0
      ? targets.avgPremiumOverride
      : undefined;

  const [showInputDialog, setShowInputDialog] = useState(false);

  // Simple (default) vs Advanced view. Persisted UI preference (same class as
  // theme/sidebar) — Simple shows the answer + key inputs; Advanced shows the
  // full optimistic/realistic math cascade.
  const [viewMode, setViewMode] = useState<"simple" | "advanced">(() => {
    if (typeof window === "undefined") return "simple";
    return localStorage.getItem("targets-view-mode") === "advanced"
      ? "advanced"
      : "simple";
  });
  const handleSetViewMode = (mode: "simple" | "advanced") => {
    setViewMode(mode);
    try {
      localStorage.setItem("targets-view-mode", mode);
    } catch {
  ...
  // Per-agent avg-premium override save (used by both Simple and Advanced).
  // null clears the override back to the computed cohort average.
  const handleSaveAvgPremiumOverride = async (value: number | null) => {
    const clean = value && value > 0 ? value : null;
    try {
      await updateTargets.mutateAsync({ avgPremiumOverride: clean });
      toast.success(
        clean ? "Average premium updated" : "Using your book average",
      );
    } catch {
      toast.error("Failed to update average premium");
    }
  };
      ...
      <div className="mx-auto w-full max-w-[2400px] px-4 py-5 lg:py-8">
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          >
            ...
            {/* Right: view toggle + inline annual target edit */}
            <div
              style={{
                display: "flex",
                alignItems: "center",
            ...
            >
              <div className="flex h-7 rounded-v2-pill border border-border overflow-hidden">
                ...
                <button
                  type="button"
                  className={cn(
                    "px-3 text-[11px] font-medium transition-colors border-l border-border",
                    viewMode === "advanced"
                      ? "bg-foreground text-background"
                      : "bg-card text-muted-foreground hover:text-foreground",
                  )}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <PillButton
                  ...
                  leadingIcon={<Edit2 className="h-3 w-3" />}
                >
                  Edit Goal
                </PillButton>
              </div>
          ...
          {viewMode === "simple" ? (
            <SimpleTargetsView
              ...
              hasPersistencyData={averages.hasData}
              persistency13Month={averages.persistency13Month}
              avgPremiumOverride={avgPremiumOverride}
              onSaveAvgPremium={handleSaveAvgPremiumOverride}
              ...
              <Board pad={20}>
                      ...
                      <PopoverContent align="start" className="w-[360px] p-3">
                        <div className="text-[13px] leading-relaxed space-y-1.5">
                          ...
                          <p className="text-muted-foreground">
                            The Optimistic plan is the raw math: gross
                            commission ÷ first-year rate ÷ avg premium. It
                            assumes every policy sticks, no taxes, and every app
                            issues.
                          </p>
                          <p className="text-muted-foreground">
                            The Realistic plan applies four haircuts so the
                            headline target reflects what you actually need to
                            do to take home your goal:
                          </p>
                          <ul className="list-disc pl-4 text-muted-foreground space-y-0.5">
                            ...
                            <li>
                              ...
                              {avgPremiumOverride
                                ? "currently overridden by your custom average premium set above."
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    ...
                    <span className="text-[11px] text-muted-foreground">
                      ...
                      {averages.hasData && averages.persistency13Month > 0 && (
                        <span className="text-[11px] text-muted-foreground/70 ml-1">
                          (13-mo:{" "}
                          {(averages.persistency13Month * 100).toFixed(0)}
                          %)
                        </span>
                      )}
                    </span>
                  ...
                  <AvgPremiumField
                    ...
                    onSaveOverride={handleSaveAvgPremiumOverride}
                  />
                </div>
              </Board>
                    ...
                    <div className="border-t pt-3 md:border-t-0 md:pt-0 md:border-l md:pl-4 border-border">
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-[13px]">
                          ...
                          >
                            {formatCurrency(
                              calculatedTargets.totalPremiumNeeded,
                            )}
                          </span>
                        </div>
                      </div>
                            ...
                            <Popover>
                              ...
                              >
                                <div className="text-[13px]">
                                  ...
                                  {avgPremiumOverride && (
                                    <div className="rounded border border-border bg-muted/40 p-2 mb-2">
                                      ...
                                      <div className="text-[11px] text-muted-foreground mt-0.5">
                                        Your custom average premium. The cohorts
                                  ...
                                  <div className="text-[11px] text-muted-foreground">
                                    {avgPremiumOverride
                                      ? "Both plans currently use your custom average premium. Clear it (Use auto) to fall back to the agency cohort (Mean/Median)."
                              ...
                              >
                                <div className="text-[13px] space-y-2">
                                  ...
                                  <p className="text-muted-foreground pt-1">
                                    Policies that lapse in year 1 contribute
                                    ~zero durable income (chargeback).
                                    Multiplying by persistency converts the
                                    first-year comp rate into the share you
                                    keep.
                                  </p>
                                </div>
                              </PopoverContent>
              ...
              >
                ...
                <Board pad={20} style={{ height: "100%" }}>
                  <div className="grid grid-cols-[1fr_auto_auto] gap-x-3 gap-y-1 items-center">
                    ...
                    <span className="text-[13px]" style={{ color: T.mut }}>
                      Annual
                    </span>
                  ...
                  >
                    ...
                    write to take home the goal after persistency, taxes &amp;
                    NTO.
                  </div>
                </Board>
              </div>
                ...
                <Board pad={20} style={{ height: "100%" }}>
                  ...
                  <div className="space-y-2">
                    ...
                    {commissionProfile?.dataQuality === "DEFAULT" && (
                      ...
                      >
                        <div className="text-[11px]" style={{ color: T.amber }}>
                          ⚠ Commission rate is using defaults. Add policies to
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <div className="flex justify-between text-[13px]">
                        ...
                        >
                          {formatPercent(
                            actualMetrics.persistency13Month * 100,
                          )}
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex justify-between text-[13px]">
                        ...
                        >
                          {formatPercent(
                            actualMetrics.persistency25Month * 100,
                          )}
                        </span>
                      </div>
                    </div>
