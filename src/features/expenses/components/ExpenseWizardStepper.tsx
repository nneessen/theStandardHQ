// src/features/expenses/components/ExpenseWizardStepper.tsx

import React from "react";
import { Check } from "lucide-react";

interface ExpenseWizardStepperProps {
  /** Ordered step labels. */
  steps: string[];
...
/**
 * Wizard stepper for the Add/Edit Expense dialog. Numbered circles across the
 * top: the active step is blue with a glow ring, completed steps show a green
 * check, and the connector lines turn green as you pass them. Visited steps are
 * clickable so the Review step's "edit-jumps" (and plain back-nav) work.
 *
 * Expense-local sibling of the policy wizard's stepper — kept separate on
 * purpose so this change never touches the freshly-shipped policy dialog.
 */
export const ExpenseWizardStepper: React.FC<ExpenseWizardStepperProps> = ({
  steps,
  current,
  furthest,
  onStepClick,
