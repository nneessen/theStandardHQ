...
// Recruiting view only: render one or more template variants (comma-separated). Empty →
// the config default. Each variant lands in its own PNG so they don't overwrite.
const VARIANTS = (process.env.VARIANTS || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
const VARIANT_LIST = VARIANTS.length ? VARIANTS : [""];
    ...
    for (const format of FORMATS) {
      ...
      try {
        ...
        for (let i = 0; i < urls.length; i++) {
          ...
          const file = path.join(outDir, `${tag}-${format}-p${i + 1}.png`);
          await writeFile(file, buf);
          const got = pngDims(buf);
          const dimsOk = got.w === want.w && got.h === want.h;
          if (!dimsOk) ok = false;
          console.log(
            `${dimsOk ? "✅" : "❌"} ${`${tag}-${format}`.padEnd(22)} slide ${i + 1}/${urls.length}  ${got.w}×${got.h}  → ${path.relative(root, file)}`,
          );
        }
        if (errors.length) {
          ok = false;
        ...
        ok = false;
        console.log(`❌ ${tag}-${format} FAILED: ${e?.message || e}`);
      }
      await page.close();
    }
  }
