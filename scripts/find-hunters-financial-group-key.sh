...
ORG_NAME="Hunter's Financial Group"

if [[ -p /dev/stdin ]]; then
  node "$SCRIPT_PATH" --stdin --org-only --match "$ORG_NAME" --show-key
  exit 0
fi

if command -v pbpaste >/dev/null 2>&1; then
  pbpaste | node "$SCRIPT_PATH" --stdin --org-only --match "$ORG_NAME" --show-key
  exit 0
fi
