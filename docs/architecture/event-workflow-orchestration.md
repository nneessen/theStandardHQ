...
- **`TenantScopedRepository<T>`** — extends `BaseRepository` for IMO-owned tables. Adds
  ...
  _"RLS remains the primary security boundary. This class makes the application default
  match that boundary so super-admin or broad RLS visibility does not accidentally leak
  records into normal screens."_
...
the `workflows` table _has_ an `imo_id` column. The match drops the tenant key that the
...
**Empirical status (verified 2026-05-25, production):** the cross-tenant _matching_ risk is
...
`policy.created`) — the missing `imo_id` predicate would then cross-match. What _is_ already
