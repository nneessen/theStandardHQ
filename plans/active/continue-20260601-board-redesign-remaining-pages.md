...
- [x] **Recruiting/Team leftovers** (commits `fc6a0800` + `92f2d5dd`): `PipelineAdminPage`, `OrgChartPage` (DRY shell+header across 4 states), `AgentDetailPage` (wrap-only, dense content kept), `OverrideDashboard` (+FlapTile hero on real summary), `DownlinePerformance` + `HierarchyManagement` (shared shell helper, all states). tsc/eslint/build green; NOT seen authed.
- [⊘] **`MyRecruitingPipeline`** (`/recruiting/my-pipeline`) — INTENTIONALLY SKIPPED. It is the recruit's own onboarding portal, rendered in its OWN full-bleed shell branch (`App.tsx` `isRecruitPipeline`, `RecruitHeader`, NO sidebar, OUTSIDE `.theme-v2`) on the warm editorial `.theme-landing` system (EditorialMasthead/Section/Stepper). Like `/command-center`, it is a deliberate separate surface — re-skinning to charcoal would be a destructive rebuild, not a re-skin. Leave as-is.
...
- [x] **Nav long-tail** (commits `1076b300`, `6ef6f073`, `ac94e979`, `0591d8ce`): Settings, Admin Control Center, Workflows, Comp Guide, Contracting, The Standard Team, Billing, Business Tools, Marketing, Orchestrator, Recruit Detail, Lead Detail, Underwriting Guides, Training Hub, Trainer Dashboard, My Training, Roadmap List, Team Progress, Close AI Builder, Lead Drop, Auth Diagnostic, Bot Health. (Mostly via parallel sonnet subagents; each verified by tsc/eslint + a final build by me.) NOT seen authed.

### DONE — board redesign effort essentially COMPLETE. Remaining = deliberate full-bleed own-surfaces only:
`/command-center`, `/recruiting/my-pipeline`, `/chat-bot`, `/voice-agent`(+clone), `/messages`, `/slack/name-leaderboard`, `/underwriting/wizard`+`/underwriting/admin`, marketing template/campaign editors, training ModulePlayer/ModuleBuilder/Presentations, agent-roadmap Runner/Editor + non-admin landing. These fill the viewport and must not be centered in the padded canvas — leave them (they already render charcoal in `.theme-v2`). Public/auth/legal routes are out of scope.
