...
### Worker → brain auth model (DECIDED)
The worker is dispatched into `jarvis-<uid>`; the human participant's `identity` is the LiveKit-signed `auth.uid()`, so the worker **trusts that user id without the model supplying it**. The worker calls a dedicated `assistant-orchestrator` entry with `Authorization: Bearer $VOICE_WORKER_SECRET` + `x-user-id: <identity>`; the orchestrator verifies the shared secret and **service-role-scopes every tool to that user id** (same RLS guarantees, no user JWT shipped through LiveKit metadata where other participants could read it). Add this path in the worker step; until then the orchestrator's normal JWT path is unchanged.
