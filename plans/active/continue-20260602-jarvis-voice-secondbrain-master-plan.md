...
1. **[DONE — deployed]** `supabase/functions/assistant-voice-livekit-token/` — mints the client's room token; identity = verified `auth.uid()` (LiveKit-signed), room `jarvis-<uid>-<session>` (per-session → multi-tab-safe). Deployed; warm-200 + 401-on-unauthorized verified; deno-checked against `livekit-server-sdk@2`.
2. **Secrets:** **[Supabase side DONE]** `LIVEKIT_API_KEY/SECRET/URL` set as Supabase function secrets (for the token fn). For the **worker** (Fly): `LIVEKIT_*`, `DEEPGRAM_API_KEY`, `ELEVENLABS_API_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`. **No `VOICE_WORKER_SECRET`** — the adopted auth model uses the user's JWT over the data channel, not a worker secret. (Owner runs `fly secrets set`; values live in `.env.local`, never echoed.)
3. **[SCAFFOLDED — type-checks vs @livekit/agents 1.4.5; NOT runtime-verified]** **Worker** (`services/jarvis-voice-worker/`, Node; commits `c9745171` → `bccf33ef`): a `voice.Agent` whose `llmNode()` override bridges each turn to `assistant-orchestrator` (SSE) with the USER's JWT (received over the data channel) → Silero VAD/turn-detector + Deepgram STT (nova-3, keyword-boosted) + ElevenLabs TTS → barge-in; `cli.runApp` auto-dispatched into `jarvis-*` rooms. `npm ci` (0 vulns) + typecheck + build green; security + supply-chain reviewed + hardened. REMAINING: `cp .env.example .env.local` + `npm run dev` against LiveKit Cloud (first runtime smoke), which needs the frontend client (#4) publishing the JWT.
